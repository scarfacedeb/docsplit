...
class ExtractTextTest < Minitest::Test
  ...
  def test_passing_extra_options_to_pdftotext
    Docsplit.extract_text('test/fixtures/obama_arts.pdf', :pages => 'all', :output => "#{OUTPUT}/htmlmeta.html", pdftotext: '-htmlmeta')
    assert File.exists?("#{OUTPUT}/htmlmeta.html")
  end
